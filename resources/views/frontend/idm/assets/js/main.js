...
;(function($, window, undefined){
	...
	$(document).ready(function()
	{
		...
		if($.isFunction($.fn.datepicker))
		{
			...
			$(".monthpicker").each(function(i, el)
			{
				var $this = $(el),
					opts = {
				...
				var $this = $(el),
					opts = {
						...
						errorPlacement: function (error, element)
						{
							...
							if(element.closest('.checkbox, .radio').length || element.parent('.input-group').length)
							{
								if(element.closest('.checkbox, .radio').length){
									error.insertAfter(element.closest('.checkbox, .radio'));
								}
								else{
		...
		{
			$(".form-wizards").each(function(i, el)
			{
				var $this = $(el),
				...
				else
				{
					...
					tile_stats.fullyEnterViewport(function(){
						...
						TweenLite.to(o, duration/1000, {curr: end, ease: Power1.easeInOut, delay: delay/1000, onUpdate: function()
							{
								$num.html(prefix + o.curr.numberFormat(0) + postfix);
							}
						});
		...
		//Load remote bootstrap modal content
		...
		// Clear bootstrap modal on close
...
// Datatable
function tableData(element, url)
{
	...
	table.dataTable({
	    ...
	    fnDrawCallback : function (oSettings) {
	        ...
	        jQuery('[data-load-remote]').on('click', function(e) {
			    ...
			    var remote = jQuery(this).data('load-remote');
			    if(remote) {
			        jQuery(jQuerythis.data('target')).load(remote);
			    }
			});
	        jQuery('#confirm-delete').on('show.bs.modal', function(e) {
	            jQuery(this).find('.btn-delete').attr('href', jQuery(e.relatedTarget).data('href'));
	        });
	    }
	});
}

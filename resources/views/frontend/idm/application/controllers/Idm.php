...
class Idm extends MY_Controller {
	private $modules = array('kegiatan');

	public function __construct()
	{
		parent::__construct();
		$this->load->model(array('kegiatan_model','idm_model'));
	}
	...
	public function index()
	{
		...
		if(!$this->permission->check_permission($this->modules, 'all')){
			...
			$this->load->model('provinsi_model');
			// $data['list_provinsi'] = $this->idm_model->get_by_criteria()
		...
		$data['list_status'] = $this->idm_model->get_by_criteria()
	...
	public function ambil_data(){
		...
		if($modul=="kab"){
			echo $this->idm_model->kabupaten($id);
		}else if($modul=="kec"){
			echo $this->idm_model->kecamatan($id);
		}else if($modul=="des"){
			echo $this->idm_model->desa($id);
		}
	}

  public function ajax_list()
    {
        $list = $this->idm_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $r) {
            $no++;
            $row = array();
                ...
                $row[] = $r->desa;
                $row[] = $r->status;
                $row[] = "<a href='idm/read/$r->id' class='btn btn-success' title='detail'><i class='fa fa-eye'></i></a>";
            $data[] = $row;
        }

        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->idm_model->count_all() ,
                        "recordsFiltered" => $this->idm_model->count_filtered(),
                        "data" => $data,
    ...
    public function read($id)
      {
          $row = $this->idm_model->get_by_id($id);
          if ($row) {
              $data = array(
		          'id' 									=> $row->id,
		          'desa'                => $row->desa,
		          ...
		          // 'skor_warung'         => $row->skor_warung,
		          // 'skor_kedai_makan'    => $row->skor_kedai_makan,
		          // 'skor_restoran'       => $row->skor_restoran,
		          // 'skor_hotel'          => $row->skor_hotel,
		          // 'skor_penginapan'     => $row->skor_penginapan,
		          ...
		          // 'skor_operasi_au'     => $row->skor_operasi_au,
		          // 'skor_jamoper'        => $row->skor_jamoper,
		          'skor_logistik'       => $row->skor_logistik,
		          // 'skor_pos_keliling'   => $row->skor_pos_keliling,
		          'skor_kredit'         => $row->skor_kredit,
		          'skor_bank'           => $row->skor_bank,
		          // 'pembuangan_limbah_ke_sungai'=> $row->pembuangan_limbah_ke_sungai,
		          // 'air'                 => $row->air,
		          // 'tanah'               => $row->tanah,
		          // 'udara'               => $row->udara,
		          // 'jumlah'              => $row->jumlah,
		          // 'rasio_pencemaran'    => $row->rasio_pencemaran,
		          'skor_kualitas_lingkungan'=> $row->skor_kualitas_lingkungan,
		          // 'tanah_longsor'       => $row->tanah_longsor,
		          // 'banjir'              => $row->banjir,
		          // 'banjir_bandang'      => $row->banjir_bandang,
		          // 'kebakaran_hutan'     => $row->kebakaran_hutan,
		          // 'rawan_bencana'       => $row->rawan_bencana,
		          // 'skor_bencana_alam'   => $row->skor_bencana_alam,
		          // 'peringatan_dini'     => $row->peringatan_dini,
		          // 'peralatan_selamat'   => $row->peralatan_selamat,
		          // 'jalur_evakuasi'      => $row->jalur_evakuasi,
		          // 'jml_kesiapan_tb'     => $row->jml_kesiapan_tb,
							'skor_bencana_alam'=> $row->skor_bencana_alam,
		          'skor_tanggap_bencana'=> $row->skor_tanggap_bencana,
		          'tahun'               => $row->tahun,
		          //'centroid_maju' => $this->idm_model->get_centroid_maju($row->provinsi),
		          );
			...
			public function gap_mandiri($id)
		    {
		        ...
		        if ($row) {
		            $data = array(
									...
									'centroid_mandiri' => $this->idm_model->get_centroid_mandiri($row->kodeprov)->result(),
				...
				public function gap_maju($id)
					{
							...
							if ($row) {
									$data = array(
										...
										'centroid_maju' => $this->idm_model->get_centroid_maju($row->kodeprov)->result(),
					...
					public function gap_berkembang($id)
				    {
				        ...
				        if ($row) {
				            $data = array(
											...
											'centroid_berkembang' => $this->idm_model->get_centroid_berkembang($row->kodeprov)->result(),
		...
		public function mandiri_nasional($id)
		{
			...
			if ($row) {
					$data = array(
						...
						'mandiri_nasional' => $this->idm_model->get_mandiri_nasional()->result(),
	...
	public function berkembang_nasional($id)
		{
			...
			if ($row) {
					$data = array(
						...
						'berkembang_nasional' => $this->idm_model->get_berkembang_nasional()->result(),
